...
```{r}
m %>% dplyr::select(-code, -year, -yearF, -den, -FR_QUERC, -FR_CONIF, -TP_SU_NO, -elev, -TP_PEND, -FR_ARBOL) %>% names()
```

```{r}
nobs_var <- nrow(m)/ncol(m %>% dplyr::select(-code, -year, -yearF, -den, -FR_QUERC, -FR_CONIF, -TP_SU_NO, -elev, -TP_PEND, -FR_ARBOL))
```
...
f <- as.formula(
  paste("den", 
      paste(
        names(
          m %>% 
            dplyr::select(-code, -year, -yearF, -den, -FR_QUERC, -FR_CONIF, -TP_SU_NO, -elev, -TP_PEND, -FR_ARBOL)),
        collapse = "+"), 
      sep = "~")
)
```
...
f <- as.formula(
  paste("den_sqrt", 
      paste(
        names(
          m %>% 
            dplyr::select(-code, -year, -yearF, -den, -den_sqrt, -FR_QUERC, -FR_CONIF, -TP_SU_NO, -elev, -TP_PEND, -FR_ARBOL, -FR_MATDE, -DS_ARBOL)),
        collapse = "+"), 
      sep = "~")
)
```
...
write.csv(as.matrix(top5_table), file=here::here("data/mod_den_sqrt_selectionBIC.csv"))
```
...
"den_sqrt ~ 1 + FR_MATDELog + HIDRO_ITH + DS_ARBOLLog" 

```{r}
f1t <- glm(den_sqrt ~ 1 + FR_MATDELog + HIDRO_ITH + DS_ARBOLLog, family = "gaussian", data = m)
