...
# Modelo seleccionado 
"riq ~ 1 + FR_MATDE + TP_RSH_V + HIDRO_ITH + elev + TP_ES_OE + p_anu_year" 

```{r}

f1 <- glm(riq ~ 1 + FR_MATDE + TP_RSH_V + HIDRO_ITH + elev + TP_ES_OE + p_anu_year, family = "gaussian", data = m)
f1.res <- simulateResiduals(f1, seed = 123, n=1000, plot=TRUE)
```
...
visreg(f1, ylab = ytitle, "FR_MATDE") 
visreg(f1, ylab = ytitle, "TP_RSH_V")
visreg(f1, ylab = ytitle, "HIDRO_ITH")
visreg(f1, ylab = ytitle, "elev")
visreg(f1, ylab = ytitle, "TP_ES_OE")
visreg(f1, ylab = ytitle, "p_anu_year")
...
summ(ms, digits = 4)
ee <- summ(ms, digits = 4)
write_csv(as.data.frame(round(ee$coeftable,4)), here::here("data/mod_riq_coefficients_SE.csv"))
```
